...
public class BuildView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra("bsId")) {
            BuildService buildService = new BuildService();
            Log.e("check", String.valueOf(buildService.getById((int) getIntent().getExtras().getLong("bsId"))));
            final List<Build> builds = SugarRecord.find(Build.class, "bs_id=?", String.valueOf(getIntent().getExtras().getLong("bsId")));
            if (!builds.isEmpty()) {
